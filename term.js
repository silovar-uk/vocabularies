(() => {
  ...
  function renderDistinctions(essay, items, catalog) {
    ...
    return '<section class="term-section">' +
      '<p class="term-kicker">DISTINCTIONS</p>' +
      '<h2>似ているけど、違う</h2>' +
      ...
      rows.map((row) => {
        ...
        return '<article class="distinction-card">' +
          '<span class="distinction-name">' + escapeHtml(label) + '</span>' +
          '<p>' + escapeHtml(row.difference) + '</p>' +
        '</article>';
      }).join("") +
      '</div></section>';
  }
  ...
  function renderSections(essay) {
    ...
    return '<section class="term-section">' +
      '<p class="term-kicker">EXPANDED ARGUMENT</p>' +
      '<h2>この言葉を使って、もう少し考える</h2>' +
      sections.map((section) =>
        '<article class="argument-block">' +
  ...
  function renderLenses(essay) {
    ...
    return '<section class="term-section">' +
      '<p class="term-kicker">LENSES</p>' +
      '<h2>この語で見る</h2>' +
      '<div class="lens-list">' +
      lenses.map((lens) =>
        '<article class="lens-card">' +
          '<span class="lens-label">' + escapeHtml(lens.label) + '</span>' +
          '<p>' + escapeHtml(lens.before) + '</p>' +
          '<p class="lens-after">' + escapeHtml(lens.after) + '</p>' +
        '</article>'
      ).join("") +
      '</div></section>';
  }
  ...
  function renderQuestions(essay) {
    ...
    return '<section class="term-section">' +
      '<p class="term-kicker">OPEN QUESTIONS</p>' +
      '<h2>ここから、さらに考えられること</h2>' +
  ...
  function renderRelations(essay, items, catalog, essayIndex) {
    ...
    return '<section class="term-section">' +
      '<p class="term-kicker">CONCEPT NETWORK</p>' +
      '<h2>この言葉から渡る</h2>' +
      ...
      relations.map((relation) => {
        ...
        return '<a class="relation-card" href="' + escapeAttr(href) + '">' +
          '<span class="relation-verb">' + escapeHtml(relation.verb || "関連する") + ' →</span>' +
          '<span class="relation-name">' + escapeHtml(names.primary || relation.term_id) + '</span>' +
        '</a>';
      }).join("") +
      '</div></section>';
  }

  function renderSources(essay) {
    const sources = essay.sources ?? [];
    if (!sources.length) return "";
    return '<section class="term-section">' +
      '<p class="term-kicker">SOURCES</p>' +
      '<h2>出典</h2>' +
      '<ul class="source-list">' +
      sources.map((source) =>
        '<li><a href="' + escapeAttr(source.url) + '" target="_blank" rel="noopener noreferrer">' +
          escapeHtml(sourceLabel(source)) + '</a></li>'
      ).join("") +
      '</ul></section>';
  }
  ...
  async function init() {
    ...
    try {
      ...
      const [collections, essay] = await Promise.all([
      ...
      page.innerHTML =
        '<article>' +
          '<header class="term-hero">' +
            '<p class="term-meta">' + escapeHtml(meta || "VOCABULARIES") + '</p>' +
            '<h1 class="term-title">' + escapeHtml(names.primary) + '</h1>' +
          ...
          '</header>' +
          '<section class="term-section term-definition">' +
            '<p class="term-kicker">30秒で掴む</p>' +
            '<p>' + escapeHtml(item.description) + '</p>' +
          '</section>' +
          renderDistinctions(essay, items, catalog) +
          renderSections(essay) +
          renderLenses(essay) +
          renderQuestions(essay) +
          renderRelations(essay, items, catalog, essayIndex) +
          renderSources(essay) +
        '</article>';
    } catch (error) {
      console.error(error);
