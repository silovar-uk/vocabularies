(() => {
  ...
  function writeAnnotations(data) {
    try {
      ...
      // Annotation is optional. Reading concepts must not depend on storage.
    }
  }

  function routes() {
    return window.VocabularyTrail?.getRoutes?.() ?? [];
  }
  ...
  function routeIdFromCard(card) {
    return card.querySelector('[data-delete-route]')?.dataset.deleteRoute || '';
  }

  function noteFor(data, routeId, index) {
    return String(data?.[routeId]?.[index] ?? '').trim();
  }

  function renderReadView(route, data) {
    const notes = route.items.map((item, index) => ({
      item,
      note: noteFor(data, route.id, index),
      index,
    })).filter((row) => row.note);

    if (!notes.length) {
      return '<p class="route-annotation-empty">まだ注釈はありません。各語をどう読んだか、なぜ次へ進んだかを残せます。</p>';
    }

    return '<div class="route-annotation-read">' + notes.map((row) =>
      '<div class="route-annotation-read-row">' +
        '<span class="route-annotation-term">' + escapeHtml(row.item.label || row.item.id) + '</span>' +
  ...
  function renderEditor(route, data) {
    return '<div class="route-annotation-editor">' +
      '<div class="route-annotation-editor-head">' +
        '<div><strong>Route Annotation</strong><span>各語に短い思考メモを残す。空欄でも保存できます。</span></div>' +
        '<span>最大' + MAX_NOTE_LENGTH + '字 / 語</span>' +
      '</div>' +
      '<div class="route-annotation-fields">' +
        route.items.map((item, index) =>
          '<label class="route-annotation-field">' +
            '<span><b>' + (index + 1) + '</b>' + escapeHtml(item.label || item.id) + '</span>' +
            '<textarea maxlength="' + MAX_NOTE_LENGTH + '" data-annotation-index="' + index + '" placeholder="例：ここで個人の負担から、環境の設計へ視点を移した。">' + escapeHtml(noteFor(data, route.id, index)) + '</textarea>' +
          '</label>'
        ).join('') +
      '</div>' +
      '<div class="route-annotation-actions">' +
        '<button type="button" data-save-annotations="' + escapeAttr(route.id) + '">注釈を保存</button>' +
        '<button type="button" data-cancel-annotations>やめる</button>' +
      '</div>' +
    '</div>';
  }
  ...
  function rerenderAll() {
    document.querySelectorAll('.concept-route-card[data-annotation-enhanced="true"]').forEach(renderCardAnnotation);
  }
  ...
  document.addEventListener('click', (event) => {
    ...
    if (save) {
      const card = save.closest('.concept-route-card');
      const section = save.closest('.route-annotation-section');
      if (card && section) saveAnnotations(save.dataset.saveAnnotations, section);
      return;
    }
