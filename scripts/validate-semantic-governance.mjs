...
function validBaseSha(value) { return /^[0-9a-f]{40}$/i.test(value ?? '') && !/^0+$/.test(value); }
...
if (validBaseSha(baseSha)) {
  ...
  if (baseCatalog) {
    ...
    if (gate.enabled && newIds.length) {
      ...
      for (const id of newIds) {
        ...
        if (!annotation) { errors.push(`${id}: 新規語にはsemantic annotationが必要です`); continue; }
        ...
        if (distance && !allowedDistances.has(distance)) errors.push(`${id}: distance=${distance} は追加不可です。許可: ${[...allowedDistances].join(', ')}`);
  ...
  } else warnings.push(`base commit ${baseSha} のcatalogを読めないため、新規語差分ゲートはスキップしました`);
}
